...
$$\alpha_t=\frac{1}{2}ln(\frac{1-\epsilon_t}{1+\epsilon_t})$$ where $E_t$ is the training error, $F_{t-1}$ is the current boosted classifier, h(x) is the output of each new weak learner and $\alpha_t$ is the weight of the new learner. To calculate $\alpha_t$, the error rate $\epsilon_m$ needs to be computed first.
...
\begin{table}[h]
  ...
  \begin{tabular}{| l |l |c |c|}
    ...
    & & Consort & Non-Consort \\
    \hline
    \parbox[t]{2mm}{\multirow{2}{*}{\rotatebox[origin=c]{90}{True}}} & Consort & 0.6 & 0.4  \\
...
\begin{table}[h]
  ...
  \begin{tabular}{| l |l |c |c|}
    ...
    & & Consort & Non-Consort \\
    \hline
    \parbox[t]{2mm}{\multirow{2}{*}{\rotatebox[origin=c]{90}{True}}} & Consort & 0.64 & 0.36  \\
                                                                     & Non-Consort & 0.34 & 0.66 \\
    \hline
  \end{tabular}
...
\begin{figure}
  ...
  \caption{Example run of forward feature selection for Gaussian SVM. Metrics
    reported are averages on the test sets averaged over a 10-fold cross validation.
